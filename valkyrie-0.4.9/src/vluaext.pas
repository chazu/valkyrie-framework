...
unit vluaext;
interface
uses sysutils, classes, variants, vlualibrary, typinfo;
...
function  vlua_callfunction(L : PLua_State; name : AnsiString; const args : Array of Variant; idx : Integer = LUA_GLOBALSINDEX) : Variant;
...
function  vlua_getpath( L: Plua_State; const path : AnsiString; idx : Integer = LUA_GLOBALSINDEX; max : Integer = -1 ) : Boolean;
function  vlua_getpath( L: Plua_State; const path : array of const; idx : Integer = LUA_GLOBALSINDEX; max : Integer = -1 ) : Boolean;
procedure vlua_register( L: Plua_State; const funcname : AnsiString; func : lua_CFunction; idx : Integer = LUA_GLOBALSINDEX );
procedure vlua_register( L: Plua_State; const lr : PluaL_Reg; idx : Integer = LUA_GLOBALSINDEX );
...
begin
  case pv^.vtype of
    ...
    vtchar       :
    begin
      if idx = LUA_GLOBALSINDEX then Exit(False);
      vlua_pushvarrec( L, pv );
      lua_gettable( L, idx-1 );
      Exit( true );
    end;
    vtString     : lua_getfield( L, idx, PChar( AnsiString(pv^.VString^) ) );
    vtPChar      : lua_getfield( L, idx, pv^.VPChar );
    vtAnsiString : lua_getfield( L, idx, PChar( AnsiString(pv^.VAnsiString) ) );
  else
    Exit(False);
  end;
  Exit( true );
...
begin
  ...
  repeat
    ...
    if Count = 0 then
      lua_getfield( L, idx, PChar(Piece) )
    else
      if lua_istable( L, -1 ) then
      begin
