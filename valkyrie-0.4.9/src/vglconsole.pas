...
unit vglconsole;
interface
uses Classes, SysUtils, vgltypes, viotypes, vioconsole, vglfontimage;

// TODO: rewrite to use indices

type TGLConsoleRenderer = class( TIOConsoleRenderer )
  constructor Create( aFontImage : TGLFontImage; aSizeX, aSizeY : DWord; aLineSpace : DWord = 0; aReqCapabilities : TIOConsoleCapSet = [VIO_CON_CURSOR] );
...
uses dateutils, vgllibrary, vmath, vutil;
...
constructor TGLConsoleRenderer.Create ( aFontImage : TGLFontImage; aSizeX, aSizeY : DWord; aLineSpace : DWord; aReqCapabilities : TIOConsoleCapSet );
var i : Byte;
begin
  ...
  FFontImage := aFontImage;
  FFontImage.SetTexCoord( EmptyTexCoord, ' ' );
  ...
  FPositionY := 0;
  FScale     := 1;
  Resize( aSizeX, aSizeY, aLineSpace );
  FStartTime := Now;
...
begin
  ...
  FCurVisible := True;
  case aType of
     VIO_CURSOR_SMALL : FFontImage.SetTexCoord( FTexCoords[ FSizeX * FSizeY ], '_' );
     VIO_CURSOR_HALF  : FFontImage.SetTexCoord( FTexCoords[ FSizeX * FSizeY ], #220 );
     VIO_CURSOR_BLOCK : FFontImage.SetTexCoord( FTexCoords[ FSizeX * FSizeY ], #219 );
  end;
...
begin
  ...
  begin // foreground rendering
    ...
    glBindTexture( GL_TEXTURE_2D, FFontImage.GLTexture );
...
begin
  ...
  GetDeviceArea.Dim := Point( FFontImage.GylphWidth * FSizeX * FScale, (FFontImage.GylphHeight + FLineSpace) * FSizeY * FScale );
end;
...
begin
  ...
  begin
    FASCII[ aIndex ] := aChar;
    FFontImage.SetTexCoord( FTexCoords[ aIndex ], aChar );
  end
  else
    aFrontColor := 0;
...
begin
  c1.x := x * FFontImage.GylphWidth;
  c2.x := c1.x + FFontImage.GylphWidth;

  if aBackGround then
  begin
    c1.y := y * ( FFontImage.GylphHeight + FLineSpace );
    c2.y := c1.y + FFontImage.GylphHeight + FLineSpace;
  end
  else
  begin
    c1.y := y * ( FFontImage.GylphHeight + FLineSpace ) + FLineSpace div 2;
    c2.y := c1.y + FFontImage.GylphHeight;
  end;
