...
type TSDLIODriver = class( TIODriver )
  class function GetCurrentResolution( var aResult : TIOPoint ) : Boolean;

  constructor Create( aWidth, aHeight, aBPP : Word; aOpenGL : Boolean = False; aFullScreen : Boolean = False );
  function ResetVideoMode( aWidth, aHeight, aBPP : Word; aOpenGL : Boolean = False; aFullScreen : Boolean = False ) : Boolean;
  ...
  function GetMousePos( var aResult : TIOPoint) : Boolean; override;
  function GetMouseButtonState( var aResult : TIOMouseButtonSet) : Boolean; override;
...
uses vgllibrary, vglulibrary,
     {Screenshot support}
     zstream, FPImage, FPCanvas, FPImgCanv,
     FPWritePNG;
...
class function TSDLIODriver.GetCurrentResolution ( var aResult : TIOPoint ) : Boolean;
...
constructor TSDLIODriver.Create( aWidth, aHeight, aBPP : Word; aOpenGL : Boolean = False; aFullscreen : Boolean = False );
begin
  ClearInterrupts;
  SDLIO := Self;
  inherited Create;
  ...
  if not ResetVideoMode( aWidth, aHeight, aBPP, aOpenGL, aFullScreen ) then
  begin
    SDL_Quit();
    SDLIO := nil;
  ...
  Log('Mode @1x@2/@3 set.', [aWidth,aHeight,aBPP]);
...
function TSDLIODriver.ResetVideoMode ( aWidth, aHeight, aBPP : Word; aOpenGL : Boolean; aFullScreen : Boolean ) : Boolean;
var iSDLFlags : DWord;
begin
  iSDLFlags := 0;
  FFScreen  := aFullScreen;
  FOpenGL   := aOpenGL;
  FSizeX    := aWidth;
  FSizeY    := aHeight;
  FBPP      := aBPP;
  ...
  if aOpenGL     then iSDLFlags := iSDLFlags or SDL_OPENGL;
  if aFullScreen then iSDLFlags := iSDLFlags or SDL_FULLSCREEN;

  Log('Checking mode @1x@2/@3bit...', [aWidth,aHeight,aBPP]);
...
function TSDLIODriver.GetMousePos ( var aResult : TIOPoint ) : Boolean;
var x,y : Integer;
begin
...
function TSDLIODriver.GetMouseButtonState ( var aResult : TIOMouseButtonSet
  ) : Boolean;
var x,y : Integer;
begin
  if SDL_GetAppState() and SDL_APPMOUSEFOCUS = 0 then Exit( False );
