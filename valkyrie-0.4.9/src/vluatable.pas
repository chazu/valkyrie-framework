...
uses sysutils, classes, variants, vlualibrary,
     vluavalue, vrltools, vutil, vnode;
...
TLuaTable = class(TVObject)
  ...
  constructor Create( L : PLua_State; const aPath : array of const; aIndex : Integer = LUA_GLOBALSINDEX );
  constructor Create( L : PLua_State; const aPath : AnsiString; aIndex : Integer = LUA_GLOBALSINDEX );
  ...
  procedure SetFlags( const aKey : AnsiString; const aValue : TFlags );
...
constructor TLuaTable.Create ( L : PLua_State; const aPath : array of const; aIndex : Integer = LUA_GLOBALSINDEX );
begin
  inherited Create;
  FClear := lua_gettop( L );
  FState := L;
  if not vlua_getpath( L, aPath ) then raise ELuaException.Create( 'Path ['+DebugToString(aPath)+'] passed to TLuaTable is nil!' );
...
constructor TLuaTable.Create ( L : PLua_State; const aPath : AnsiString; aIndex : Integer = LUA_GLOBALSINDEX );
begin
  inherited Create;
  FClear := lua_gettop( L );
  FState := L;
  if not vlua_getpath( L, aPath ) then raise ELuaException.Create( 'Path ['+aPath+'] passed to TLuaTable is nil!' );
  if not lua_istable( L, -1 ) then
  begin
    lua_pop( L, 1 );
...
begin
  ...
  Result := vlua_toflags_flat( FState, -1 );
  Reset;
end;
...
begin
  ...
  if TryGetField( aKey, LUA_TTABLE )
    then Result := vlua_toflags_flat( FState, -1 )
    else Result := aDefault;
  Reset;
end;
...
procedure TLuaTable.SetFlags ( const aKey : AnsiString; const aValue : TFlags ) ;
begin
  Push;
  vlua_pushflags_flat( FState, aValue );
  lua_setfield( FState, -2, PChar(aKey) );
  Reset;
end;
