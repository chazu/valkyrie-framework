...
begin
  ...
  begin
    ...
    Log('Setting terminal video to @1x@2 color...',[aCols,aRows]);
    if not SetVideoMode(iVideoMode)
       then Log('Failed to set terminal @1x@2 color mode.',[aCols,aRows]);
  end;
  ...
  if iVideoMode.Color then
    Log('Terminal video mode - @1x@2 color.',[FSizeX,FSizeY])
  else
    Log('Terminal video mode - @1x@2 NO COLOR.',[FSizeX,FSizeY]);
...
function TTextIODriver.GetMousePos ( var aResult : TIOPoint ) : Boolean;
begin
...
function TTextIODriver.GetMouseButtonState ( var aResult : TIOMouseButtonSet
  ) : Boolean;
begin
  if not FMouse then Exit( False );
